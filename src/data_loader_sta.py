...
class CharadesSTADataset(Dataset):
    def __init__(self, annotation_path, video_features_dir, max_seq_len=64, is_train=True):
        """
        Args:
        ...
        self.video_lengths = {}
        for csv_path in [
            config['paths']['charades_v1_train'], #Update code to read paths from a config .yaml file
    ...
    def __getitem__(self, idx):
        ...
        #Map timestamps to frame indices correctly based on actual video duration, features are uniformly sampled across the video length
        ...
        #Pad or truncate features (Truncation may lead to loss of information)
