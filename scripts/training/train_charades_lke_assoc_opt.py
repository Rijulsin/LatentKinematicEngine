...
CHECKPOINT_PATH = os.path.join(CHECKPOINT_DIR, 'best_charades_sta_lke_assoc_opt_model.pt')
...
def run_epoch(model, dataloader, criterion, clip_model, tokenizer, nlp, device,
    ...
    for batch in dataloader:
        ...
        if train:
            # Temporal Feature Masking (randomly zero out 10% of frames)
            mask_drop = (torch.rand(video_dense.shape[:-1], device=device) > 0.10).unsqueeze(-1)
            video_dense = video_dense * mask_drop
...
def main():
    ...
    print("\n--- INITIALIZING LATENT KINEMATIC ENGINE (OPTIMIZED) ---", flush=True)
    model = LatentKinematicGroundingMemory(d_video=768, d_text=512, d_model=512, num_layers=4).to(device)
    # Tuned Loss Weights: Increased importance of GiOU and Contrastive alignment
    criterion = BoundaryLoss(lambda_iou=0.6, lambda_contrast=0.4).to(device)
    # Tuned Optimizer: Lower LR for stable fine-tuning, higher weight decay for regularization
    optimizer = optim.AdamW(model.parameters(), lr=8e-5, weight_decay=0.05)
