...
def main():
    ...
    with torch.no_grad():
        for batch in tqdm(test_dataloader, desc="Evaluating"):
            ...
            gt_s_sec = batch['start_time'].item()
            gt_e_sec = batch['end_time'].item()
            vid_dur = batch['video_duration'].item()
            ...
            cont_iou = temporal_iou(pred_s_sec, pred_e_sec, gt_s_sec, gt_e_sec)
    ...
    print(f"Discrete mIoU: {mIoU:.2f}% | Continuous mIoU: {cont_mIoU:.2f}%")
    print(f"Discrete Recall@1 (IoU=0.5): {r_05:.2f}% | Continuous: {cont_r_05:.2f}%")
    print(f"Discrete Recall@1 (IoU=0.7): {r_07:.2f}% | Continuous: {cont_r_07:.2f}%")
