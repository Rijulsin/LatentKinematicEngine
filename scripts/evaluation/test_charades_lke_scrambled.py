...
def main():
    ...
    # Pre-load all queries to shuffle them
    ...
    with torch.no_grad():
        for batch in test_dataloader:
            ...
            # REPLACE WITH SHUFFLED QUERIES
            batch_size = len(batch['query'])
            shuffled_batch_queries = all_queries[query_idx : query_idx + batch_size]
            query_idx += batch_size
            
            noun_queries = []
            verb_queries = []
            for query in shuffled_batch_queries:
    ...
    print(f"SHUFFLED QUERIES: Continuous mIoU: {cont_mIoU:.2f}% | Continuous Recall@1@0.5: {cont_r_05:.2f}%")
